  ...
  loadJSONData(function (response) {
    ...
    responseData.forEach((responseItem) => {
      let newArticle = new Article(
        responseItem.id,
        new Date(responseItem.date),
        responseItem.title,
        responseItem.preview,
        responseItem.boldInfo,
        responseItem.info,
        responseItem.image,
        responseItem.views
      );
...
const renderSvg = (articleImageContainer, article) => {
  ...
  iconPath.setAttribute(
    "d",
    "M6 21a1 1 0 0 1-.49-.13A1 1 0 0 1 5 20V5.33A2.28 2.28 0 0 1 7.2 3h9.6A2.28 2.28 0 0 1 19 5.33V20a1 1 0 0 1-.5.86 1 1 0 0 1-1 0l-5.67-3.21-5.33 3.2A1 1 0 0 1 6 21z"
  );
...
const createArticlesContainer = (articleArray) => {
  ...
  articleArray.forEach((article) => {
    ...
    articleDate.textContent = article.date.toLocaleDateString("en-us", {
      year: "numeric",
      month: "long",
      day: "numeric",
    });
...
const checkBigArticle = () => {
  if (
    articlesSorted ||
    articleSearchBar.value.length > 2 ||
    viewingSavedArticles
  ) {
...
const searchArticles = () => {
  ...
  if (articleSearchBar.value.length > 2) {
    articles.forEach((article) => {
      if (
        !article.childNodes[1].childNodes[1].textContent
          .toLowerCase()
          .includes(articleSearchBar.value.toLowerCase())
      ) {
